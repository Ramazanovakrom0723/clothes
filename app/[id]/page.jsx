...
import { getSingleProduct } from '../../service/auth.product';
    ...
    <div className="p-4 pt-[200px] md:px-[140px] bg-gray-100">
      <div className="flex container flex-col md:flex-row gap-8 p-6 rounded-lg">
        <div className="md:w-1/2">
          ...
          >
            {data.image_url && data.image_url.map((item, index) => (
              <SwiperSlide
              ...
              >
                <Image
                  ...
                  className="w-[370px] py-10 h-[370px] object-cover"
                  width={370}
                  height={370}
                />
              </SwiperSlide>
          ...
          <div className="flex gap-2 mt-4">
            {data.image_urls && data.image_urls.map((item, index) => (
              <div key={index} className="w-20 bg-white p-2 h-20 border-2 border-yellow-500 rounded-lg overflow-hidden">
                <Image src={item} alt={`Thumbnail image ${index + 1}`} className="w-full h-full object-cover" width={80} height={80} />
              </div>
            ))}
          </div>
        </div>
        <div className="md:w-1/2 p-8 bg-white">
          ...
          <p className="mt-2 text-gray-700">В наличии: {data.count} шт.</p>
          <p className="mt-2 text-gray-700">Страна производитель: {data.made_in}</p>
          <p className="mt-4 text-2xl font-bold">{data.cost} UZS / 1 шт.</p>
          <div className="flex gap-4 mt-4">
            <button className="px-4 py-2 bg-yellow-400 text-black rounded-md">
              <i className="bi bi-cart3 mr-1"></i> Корзина
            </button>
          ...
          <div className="mt-4">
            <button className="px-4 py-2 border-2 mt-24 rounded-md">
              <i className="bi bi-share mr-1"></i> Поделиться
            </button>
          </div>
        </div>
      </div>
      <div className="mt-28 grid grid-cols-1 container md:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-lg shadow-md xl:w-[600px] sm:w-[300px] xl:h-[500px] sm:h-[400px] xl:py-[40px] sm:py-[20px] xl:px-[40px] sm:px-[20px]">
          ...
          <ul className="flex flex-wrap gap-4">
            {data.attributes && data.attributes.map((attr, index) => (
              <li className="xl:w-[150px] sm:w-[100px]" key={index}>
                <h3 className="sm:text-[12px] xl:text-[16px]">{attr.name}:</h3>
                <p className="sm:text-[10px] xl:text-[16px]">{attr.value}</p>
              </li>
            ))}
          </ul>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          {data.reviews && data.reviews.map((review, index) => (
            <div className="mt-4 flex gap-4" key={index}>
              ...
              <div>
                ...
                <p className="text-gray-700">{review.role}</p>
          ...
          <a href="#" className="mt-4 text-yellow-400">
            <i className="bi bi-prev"></i> Все отзывы
          </a>
        </div>
      </div>
